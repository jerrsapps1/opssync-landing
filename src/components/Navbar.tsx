...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/80">
      <div className="container-ss flex items-center justify-between py-1.5">
        <Link to="/" className="flex items-center gap-2">
          <BrandMark size={16}/>
          <span className="text-[13px] font-extrabold tracking-tight">OpsSync.ai</span>
        </Link>
      ...
      {open && (
        <div className="border-t bg-white/90 py-2 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/90 md:hidden">
          <div className="container-ss grid gap-1">
            ...
            <Link to="/pricing" onClick={()=>setOpen(false)} className="btn btn-primary mt-1">Get started</Link>
          </div>
        </div>
      )}
    </nav>
